...
export async function fetchDashboardAnalytics(userId, options = {}) {
  const days = parseDays(options.days ?? 30, 30);
  ...
  return buildAnalytics(metrics, workouts, meals, sleep, days);
}

export async function generateUserRecommendations(userId, options = {}) {
  const lookbackDays = parseDays(options.days ?? 14, 14);
  ...
  return generateRecommendations(metrics, workouts, meals, sleep);
}
...
export async function calculateUserAchievements(userId) {
  ...
  return calculateAchievements(metrics, workouts, meals, sleep);
}

function buildAnalytics(metrics, workouts, meals, sleep, days) {
  return {
    summary: {
      totalDays: days,
      activeDays: new Set([
    ...
    fitness: {
      ...
      averageWorkoutDuration:
        workouts.length > 0
          ? Math.round(workouts.reduce((sum, w) => sum + w.durationMin, 0) / workouts.length)
          : 0,
...
function getBestSleepDay(sleep) {
  ...
  const bestSession = sleep.reduce((best, session) => {
    const duration = (new Date(session.endTime) - new Date(session.startTime)) / (1000 * 60 * 60);
    const bestDuration =
      best != null ? (new Date(best.endTime) - new Date(best.startTime)) / (1000 * 60 * 60) : 0;

    if (duration > bestDuration && session.quality === "excellent") {
      return session;
    }
    return best;
  }, null);
...
function calculateWeeklyProgress(metrics, workouts, meals, sleep) {
  ...
  [...metrics, ...workouts, ...meals, ...sleep].forEach((item) => {
    ...
    if (item.calories !== undefined && item.mealType) {
      weeks[weekKey].meals += 1;
    }
    if (item.startTime && item.endTime) {
      weeks[weekKey].sleep += 1;
    }
    if (item.steps !== undefined) {
      weeks[weekKey].metrics += 1;
    }
  });
